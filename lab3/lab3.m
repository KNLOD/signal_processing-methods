...
%Создание 3-го поля
hAxes3=axes('Units', 'pixels', 'Position', [x1+2*dx+2*w, y1, w, h]);
%Создание 4-го поля
hAxes4=axes('Units', 'pixels', 'Position', [x1, y1+dy+h, w, h]);
%Создание 5-го поля
hAxes5=axes('Units', 'pixels', 'Position', [x1+dx+w, y1+dy+h, w, h]);
%Создание 6-го поля
hAxes6=axes('Units', 'pixels', 'Position', [x1+2*dx+2*w, y1+dy+h, w, h]);
%Создание 7-го поля
hAxes7=axes('Units', 'pixels', 'Position', [x1, y1+2*dy+2*h, w, h]);
%Создание 8-го поля
hAxes8=axes('Units', 'pixels', 'Position', [x1+dx+w, y1+2*dy+2*h, w, h]);
%Создание 9-го поля
hAxes9=axes('Units', 'pixels', 'Position', [x1+2*dx+2*w, y1+2*dy+2*h, w, h]);
...
set(hAxes2, 'YLim', [-Asum  +Asum]); % Установка пределов +-Asum
hold on             
